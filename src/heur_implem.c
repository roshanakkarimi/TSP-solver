...
double greedy_picker(int i, int* sol, const instance* inst){ /*just a wrapper to use node_picker standard*/
	return minCost(i, sol, inst);
} /*greedy_picker*/

double grasp_picker(int i, int* sol, const instance* inst){
	if(rand() % 100 < inst->prob)
		return minCost(i, sol, inst);
	else 
...
void refine(instance* inst, int* sol, bool outF){ 
	...
	if(inst->zbest == INFINITE_DBL){
		myError("No refinement without any update of zbest!", SKIP_REF_ERR);
		exit(1);
	} /*to correct*/
